...
#+end_example
* 启用 PF
_pf 模块_ 随基本系统一同发布。启用 PF 前，需要先加载内核模块：
...
#+begin_example
  若 PF 内核模块未加载，运行 pfctl 时会提示

  pfctl: /dev/pf: No such file or directory
#+end_example

默认情况下，PF 从 _/etc/pf.conf_ 读取其配置规则，并根据该文件中指定的规则或定义修改、丢弃或放行数据包。复制示例文件作为默认配置规则集，否则 pf 无法启动：
...
若 PF 找不到其规则集配置文件，将无法启动

#+begin_example
  默认情况下，FreeBSD 未附带规则集，也没有 /etc/pf.conf 文件

  可在 /usr/share/examples/pf/ 中找到示例规则集
#+end_example

若自定义规则集保存于其他位置，可在 /etc/rc.conf 中添加一行，指定文件的完整路径：
...
要控制 PF，可使用 _pfctl_ 。以下总结了该命令的一些常用选项
...
#+begin_example
有关所有可用选项的说明，参阅 pfctl(8)
#+end_example

为监控通过 PF 防火墙的流量，可考虑安装软件包或 _sysutils/pftop_ 。安装后，可运行 _pftop_ 查看流量的实时快照，其格式类似于 top(1) 
* PF 规则集
...
1. 第一条规则 _block in all_ 默认拒绝所有传入的流量
2. 第二条规则 _pass out all keep state_ 允许放行由该系统发起的连接，同时保留这些连接的状态信息
   + 这些状态信息允许该连接的返回流量通过，且此功能应仅在受信任的计算机上使用
   + 在 PF 中，pass 规则默认隐式创建状态，因此 keep state 可省略
...
PF 识别端口名以及端口号，只要在 /etc/services 中列出了这些名称
#+begin_example
  此例创建了两个宏，第一个是五个 TCP 端口名的列表 ssh, submission, domain, www, https

  第二个是一个 UDP 端口名 domain
#+end_example

定义后，即可在规则中使用宏。在此例中，除了该系统发起的五个指定的 TCP 服务连接和一个指定的 UDP 服务连接外，所有流量都会被阻止：
...
尽管 UDP 通常被视为无状态协议，但 PF 能够跟踪某些状态信息
#+begin_example
例如，当发送 UDP 请求查询域名服务器时，PF 会监控返回流量以放行
#+end_example
...
指定 _-n_ 选项会使规则仅被解释而不被加载，便于纠正错误。任何时候，最后加载的有效规则集均会生效，直至 PF 被禁用或加载新规则集

#+begin_example
  在 pfctl 规则集验证或加载时添加 -v 选项，会显示完全解析后的规则，准确展示其加载方式
...
接下来，创建 PF 规则以允许网关转发流量。以下规则允许来自内部网络主机的有状态流量通过网关，但 _to_ 关键字并不保证数据包能从源端完整到达目的端：
...
此规则集使用 _nat-to_ 选项将内部网络中不可路由的地址转换为分配给外部接口的 IP 地址，即 *网络地址转换* _NAT_
...
注意此规则中的 _quick_ 关键字。由于规则集中包含多个规则，理解规则之间的关系非常重要
...
#+end_example
在需要使用 FTP 的情况下，PF 可将 FTP 流量重定向至小型代理程序 _ftp-proxy(8)_ ，该程序包含于 FreeBSD 基本系统中。代理的作用是 *动态* 插入和删除规则集中的规则，通过一组 _锚点_ 以正确处理 FTP 流量。
...
#+end_example
ICMP 协议在主机和网关之间发送和接收 *控制消息* _Control Message_ ，主要是向发送方提供有关到达目标主机途中出现的异常或困难的反馈
+ 路由器使用 ICMP 协商数据包大小和其他传输参数，此过程通常称为 *路径 MTU 发现* _Path MTU Discovery_ 
...
#+begin_example
  互联网协议在设计上与设备无关，设备无关性导致的一个后果是，给定连接的最佳数据包大小并非总能可靠预测

  数据包大小的主要限制是 最大传输单元（MTU），它设置了接口的最大数据包大小限制

  可输入 ifconfig 以查看系统网络接口的 MTU 值
#+end_example
TCP/IP 使用一种称为 _路径 MTU 发现_ 的过程来确定连接的正确数据包大小。此过程会发送不同大小的数据包，并设置“禁止分片”标志，期望在达到上限时接收到类型为“3，代码 4”的 ICMP 返回包
+ 类型 3 表示“目的地不可达”
+ 代码 4 表示“需分片，但已设置禁止分片标志”


若需要允许路径 MTU 发现以支持连接至不同 MTU 的链路，需要将 destination unreachable 类型添加到 icmp_types 宏中
#+begin_src sh 
...
#+begin_example
  PF 允许基于所有变种的 ICMP 类型和代码过滤

  可能的类型和代码列表在 icmp(4) 和 icmp6(4) 中有说明
#+end_example
** 使用表
#+begin_example
  有些类型的数据在某些时候对过滤和重定向非常重要，但它们的定义太长，无法包含在规则集文件中
#+end_example
PF 支持使用表，表是可在不重新加载整个规则集的情况下操作的定义列表，且支持快速查找。表名总是被 _< >_ 括起来，如下所示：
...
可以使用 _pfctl_ 动态操作表的内容。以下示例将另一个网络添加到表中：
...
#+begin_example
  ...
  若需要允许合法用户通过外部 SSH 访问，更改 SSH 使用的默认端口可提供一定的保护

  PF 提供了一种更优雅的解决方案。通过限制连接主机的操作，违规者将被移入地址表

  表中地址将被拒绝部分或全部访问权限。甚至可丢弃超限机器的所有现有连接
#+end_example
...
+ max-src-conn: 允许从单个主机发起的同时连接的数量
+ max-src-conn-rate: 允许从任何单个主机每 5 秒发起的新连接速率 15 
+ overload <bruteforce>: 表示任何超出这些限制的主机，其地址将被添加到 _bruteforce_ 表中
  + 规则集会阻止来自 bruteforce 表中地址的所有流量
+ flush global: 表示当主机达到限制时，所有 _global_ 该主机的连接将被终止 _flush_

#+begin_example
  这些规则 不会 阻止慢速暴力破解者，如在 http://home.nuug.no/~peter/hailmary2013/ 中所述
#+end_example

例如，若希望允许大量连接，但希望在 SSH 方面更严格，可在规则集的较早位置使用类似以下的规则补充：
...
随着时间的推移，表将被超载规则填充，大小将逐渐增大，占用更多内存

#+begin_example
有时被阻止的 IP 地址为动态分配地址，该地址可能已重新分配给有正当理由与本地网络主机通信的主机
#+end_example
...
_block-policy_ 是可在规则集的 options 部分设置的选项，该部分位于重定向和过滤规则之前。此选项决定了当规则阻止主机时，PF 是否会发送反馈。该选项有两个可能的值：
...
在 PF 中， _scrub_ 是一个启用网络数据包规范化的关键字。该过程会丢弃具有无效标志组合的 TCP 数据包
...
对于分片重组，推荐在规则集的 options 部分使用 _set reassemble yes_ ：
...
_antispoof_ 机制防止伪造或伪装的 IP 地址攻击，主要通过阻止在特定接口和方向上逻辑上不可能出现的数据包来实现。这些规则会过滤来自外部世界的伪造流量，以及源自本地网络的任何伪造数据包：
...
* 其他 PF 功能
** SYN Cookie 防护
PF 提供了 _set syncookies_ 选项，用于防御 SYN 洪水攻击。当 SYN cookie 激活时，PF 对每个入站 TCP SYN 回复 syncookie SYNACK，而不分配任何资源。收到客户端的 ACK 后，PF 评估规则集并在允许时创建状态:
#+begin_src sh 
  # 从不发送 syncookie（默认行为）
...
** 包速率限制
_max-pkt-rate_ 选项可基于规则限制数据包速率。超过指定速率时，规则停止匹配：
...
#+ATTR_HTML: :border 1 :rules all :frame boader
|[[file:fail2ban.org][Next: Fail2Ban]] | [[file:ipf.org][Previous: IPfilter]] |  [[file:firewall.org][Home: 防火墙]] |
