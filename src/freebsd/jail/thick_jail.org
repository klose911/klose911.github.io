...
运行以下命令为 Jail 安装基本系统，建议使用 _base.txz_ 解压安装，省去编译的时间
...
首先配置宿主机网络。创建 *网桥* _bridge0_ 和 *虚拟以太网对* _epair_ ，供 *VNET* _Virtualized Network Stack 虚拟网络栈_ 使用，然后将 _虚拟网卡 epair0a_  *挂载* 到 _网桥_ 。编辑 /etc/rc.conf 文件：
...
_bridge0_ 是 *虚拟交换机* _(Network Bridge_  ， _epair0a_ 是虚拟交换机上的一个 *端口*
#+begin_example
...
+ 配置中前三行允许在 Jail 内部使用 *System V IPC* _进程间通信_ 机制，包括 _共享内存_ 、 _信号量_ 和 _消息队列_ ；
...
启动以后使用 _jls_ 命令查看，若容器创建成功，且正在运行，该容器将出现在列表中
...
执行 _jexec myjail sh_ 进入容器命令行（此操作需要 Root 权限），成功后将看到命令提示符 #，此时可输入命令操作容器。若已按照前面的方法允许容器创建原始套接字，可使用 ping example.com 确认已经连接到互联网:
...
#+ATTR_HTML: :border 1 :rules all :frame boader
| [[file:qjail.org][Next: Qjail]] | [[file:basic.org][Previous: Jail 概述]] | [[file:jail.org][Home: Jail 容器管理]] |
