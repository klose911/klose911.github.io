...
** 包含 ssh 密钥的 pod
  ...
  $ kubectl create secret generic ssh-key-secret --from-file=ssh-privatekey=/path/to/.ssh/id_rsa --from-file=ssh-publickey=/path/to/.ssh/id_rsa.pub
   ...
   #+BEGIN_EXAMPLE
     发送自己的 ssh 密钥之前要仔细思考：集群的其他用户可能有权访问该密钥。使用想要共享 Kubernetes 群集的所有用户可以访问的服务帐户，如果它们遭到入侵，可以撤销
   #+END_EXAMPLE

   现在可以创建一个使用 ssh 密钥引用 secret 的pod，并在一个卷中使用它：
...
** 包含 prod/test 凭据的 pod
  ...
  /etc/secret-volume/password
   ...
   #+BEGIN_EXAMPLE
     注意：两个 pod 的 spec 配置中仅有一个字段有所不同，这有助于使用普通的 pod 配置模板创建具有不同功能的 pod
   #+END_EXAMPLE

   可以使用两个 service account 进一步简化基本 pod spec：一个名为 prod-user 拥有 prod-db-secret ，另一个称为 test-user 拥有 test-db-secret 。然后，pod spec 可以缩短为，例如：
...
** Secret 仅对 pod 中的一个容器可见
   #+BEGIN_EXAMPLE
     考虑以下一个需要处理 HTTP 请求的程序，执行一些复杂的业务逻辑，然后使用 HMAC 签署一些消息

     因为它具有复杂的应用程序逻辑，所以在服务器中可能会出现一个未被注意的远程文件读取漏洞，这可能会将私钥暴露给攻击者
   #+END_EXAMPLE

   这可以在两个容器中分为两个进程：
   1. 前端容器，用于处理用户交互和业务逻辑，但无法看到私钥
   2. 可以看到私钥的签名者容器，并且响应来自前端的简单签名请求
      #+BEGIN_EXAMPLE
	例如通过本地主机网络
      #+END_EXAMPLE
...
** 客户端使用 secret API
   + 当部署与 secret API 交互的应用程序时，使用诸如 RBAC 之类的 _授权策略_ 来限制访问
   ...
   + 由于这些原因，在命名空间中 watch 和 list secret 的请求是非常强大的功能，应该 *避免* 这样的行为
     #+BEGIN_EXAMPLE
       因为列出 secret 可以让客户端检查所有 secret 是否在该命名空间中

       在群集中watch 和 list 所有 secret 的能力应该只保留给最有特权的系统级组件
     #+END_EXAMPLE
   + 需要访问 secrets API 的应用程序应该根据他们需要的 secret 执行 get 请求
     #+BEGIN_EXAMPLE
       这允许管理员限制对所有 secret 的访问，同时设置 白名单访问 应用程序需要的各个实例
     #+END_EXAMPLE
...
** 保护
   1. 因为 secret 对象可以独立于使用它们的 pod 而创建，所以在创建、查看和编辑 pod 的流程中 secret 被暴露的风险较小
      #+BEGIN_EXAMPLE
	系统还可以对 secret 对象采取额外的预防措施，例如避免将其写入到磁盘中可能的位置
      #+END_EXAMPLE
   2. 只有当节点上的 pod 需要用到该 secret 时，该 secret 才会被发送到该节点上。它不会被写入磁盘，而是存储在 tmpfs 中。一旦依赖于它的 pod 被删除，它就被删除
   3. 在大多数 Kubernetes 项目维护的发行版中，用户与 API server 之间的通信以及从 API server 到 kubelet 的通信都受到 SSL/TLS 的保护
      #+BEGIN_EXAMPLE
	通过这些通道传输时，secret 受到保护
      #+END_EXAMPLE
   4. 节点上的 secret 数据存储在 tmpfs 卷中，因此不会传到节点上的其他磁盘
   5. 同一节点上的很多个 pod 可能拥有多个 secret。但是，只有 pod 请求的 secret 在其容器中才是可见的
      #+BEGIN_EXAMPLE
	因此，一个 pod 不能访问另一个 Pod 的 secret
      #+END_EXAMPLE
   6. Pod 中有多个容器。但是，pod 中的每个容器必须请求其挂载卷中的 secret 卷才能在容器内可见
      #+BEGIN_EXAMPLE
	这可以用于 在 Pod 级别构建安全分区
      #+END_EXAMPLE
** 风险
   + API server 的 secret 数据以纯文本的方式存储在 etcd 中；因此：
     + 管理员应该限制 admin 用户访问 etcd
     + API server 中的 secret 数据位于 etcd 使用的磁盘上：管理员在不再使用时擦除/粉碎 etcd 使用的磁盘
   + 如果将 secret 数据编码为 base64 的清单（JSON 或 YAML）文件，共享该文件或将其检入代码库，这样的话该密码将会被泄露
     #+BEGIN_EXAMPLE
   ...
   + 目前，任何节点的 root 用户都可以通过模拟 kubelet 来读取 API server 中的任何 secret
     #+BEGIN_EXAMPLE
       只有向实际需要它们的节点发送 secret 才能限制单个节点的根漏洞的影响
     #+END_EXAMPLE

| [[file:resource-limit.org][Next：资源配额]] | [[file:service-account.org][Previous: Service Account]] | [[file:config.org][Home：配置]] |
